...
flowchart TD
    GOAL{"먼저 만들고 싶은 것"}
    GOAL -->|자동화/AI 실습| PY["Python"]
    GOAL -->|웹 화면| JS["JavaScript"]
    GOAL -->|메모리/시스템| C["C"]
    GOAL -->|서버/OOP 구조| JAVA["Java"]
    GOAL -->|성능+추상화| CPP["C++"]
    PY --> CORE["공통 CS 개념"]
    JS --> CORE
    C --> CORE
...
언어 선택은 정답 맞히기가 아니라 비용 배분이다. Python으로 시작하면 메모리 세부는 늦게 배우지만 실습 속도가 빠르다. C로 시작하면 결과를 보기까지는 느리지만 메모리·컴파일·표현을 일찍 만난다. JavaScript로 시작하면 웹 플랫폼을 빨리 만나는 대신 브라우저와 언어 자체의 경계를 함께 배워야 한다.
...
| 목표 | 추천 시작 | 보강 경로 |
|---|---|---|
| 데이터 분석과 ML을 빨리 해 보고 싶다 | Python | 이후 C/C++로 성능·메모리 보강 |
| 웹 서비스를 직접 눈으로 보고 싶다 | JavaScript | 이후 TypeScript, 네트워크, 백엔드 |
| OS와 메모리를 이해하고 싶다 | C | 이후 Rust/C++, 운영체제 |
| 큰 서버 애플리케이션 구조를 배우고 싶다 | Java | 이후 Spring, DB, 시스템 설계 |
| 게임/엔진/고성능을 목표로 한다 | C++ | 이후 컴퓨터 구조, 성능 공학 |
...
언어 성능은 상수 계수와 런타임 특성의 차이다. 알고리즘 복잡도가 `O(n^2)`에서 `O(n log n)`으로 바뀌는 효과는 대부분 언어 선택보다 크다.
