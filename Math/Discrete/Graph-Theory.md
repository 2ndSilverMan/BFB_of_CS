...
그래프를 모델링할 때는 "정점이 무엇이고, 간선이 무엇을 뜻하는가"를 먼저 정해야 한다. 같은 현실 문제도 정점 정의가 달라지면 완전히 다른 그래프가 된다.
...
| 관계에 방향이 있는가? | 무방향 / 방향 |
| 간선에 비용이 있는가? | 비가중 / 가중 |
| 같은 두 정점 사이 여러 간선이 가능한가? | 단순 그래프 / 다중 그래프 |
| 자기 자신으로 가는 간선이 가능한가? | loop 허용 여부 |
| 모든 정점을 지나야 하는가, 특정 두 정점만 관심인가? | 탐색 / 최단 경로 / 연결성 |

예를 들어 친구 관계는 보통 무방향 그래프지만, 팔로우 관계는 방향 그래프다. 도로망은 거리나 시간 가중치가 붙고, 일방통행이면 방향까지 필요하다.

### 트리의 동치 조건

정점 수가 $n$인 무방향 그래프에서 다음 조건들은 서로 강하게 연결된다.

| 조건 | 의미 |
|---|---|
| 연결이고 사이클 없음 | 트리의 정의 |
| 연결이고 간선 수 $n-1$ | 최소 연결 구조 |
| 사이클이 없고 간선 수 $n-1$ | 모든 정점이 연결됨 |
| 임의 두 정점 사이 단순 경로가 유일 | 우회 경로가 없음 |

알고리즘에서 DFS tree, MST, union-find의 cycle detection이 모두 이 성질을 사용한다.

### 방향 그래프와 DAG

방향 그래프에서 사이클이 없으면 DAG(directed acyclic graph)다. DAG는 작업 의존성, 빌드 시스템, 계산 그래프를 표현한다. DAG에서는 위상 정렬이 가능하며, 간선 $u\to v$는 보통 "$u$가 $v$보다 먼저 와야 한다"는 뜻이다.
...
무방향 그래프의 간선 수는 차수 합을 2로 나누면 된다.
...
def has_self_loop(graph):
    return any(v in neighbors for v, neighbors in graph.items())
```
...
BFS/DFS는 인접 리스트 기준 `O(V + E)`에 모든 정점과 간선을 한 번씩 본다. 인접 행렬에서는 이웃 확인에 각 정점마다 `O(V)`를 쓰므로 전체가 `O(V^2)`가 된다.
...
- 최단 경로 문제에서 간선 수가 적다는 것과 가중치가 작다는 것은 다르다. BFS는 비가중 그래프에서만 거리 최단을 보장한다.
- DAG의 "방향을 무시하면 사이클이 없어야 한다"는 뜻이 아니다. 방향을 따른 사이클이 없어야 한다.
...
- 친구 관계와 팔로우 관계를 각각 그래프로 모델링하고 방향성 차이를 설명하라.
- DAG 예제를 하나 만들고 가능한 위상 정렬을 두 개 이상 찾아라.
