...
```mermaid
flowchart LR
    X["원본 데이터 X"] --> CENTER["학습 평균으로 중심화"]
    CENTER --> SVD["SVD / 공분산 고유분해"]
    SVD --> KEEP["상위 k개 주성분 선택"]
    KEEP --> Z["저차원 점수 Z"]
    Z --> MODEL["시각화, 압축, 모델 입력"]
```
...
| 관점 | 최적화 대상 |
|---|---|
| 분산 최대화 | 투영된 데이터 $X_cV_k$의 분산을 최대화 |
| 재구성 오차 최소화 | $\|X_c-X_cV_kV_k^\top\|_F^2$를 최소화 |

그래서 PCA는 "가장 많이 흔들리는 축을 찾는 방법"이면서 동시에 "선형 압축 후 되살렸을 때 평균 제곱 오차가 가장 작은 방법"이다. 이 동치는 SVD의 최적 저랭크 근사 성질에서 나온다.

### 성분 수 선택

설명 분산 비율은 각 성분이 전체 분산 중 얼마를 차지하는지 나타낸다.

$$
\text{ratio}_i=\frac{\sigma_i^2}{\sum_j\sigma_j^2}
$$

누적 설명 분산이 90% 또는 95%를 넘는 최소 $k$를 고르는 방식이 흔하지만, 항상 정답은 아니다. 시각화는 $k=2$나 $3$이 목적에 맞고, 압축은 저장 비용과 품질의 균형을 봐야 하며, 지도학습 전처리는 검증 성능으로 $k$를 선택해야 한다.

### 데이터 누출 방지 절차

PCA를 모델링 파이프라인에 넣을 때는 다음 순서를 지킨다.

1. 학습 데이터에서만 평균, 표준편차, 주성분을 추정한다.
2. 검증/테스트 데이터에는 학습 데이터에서 저장한 평균과 주성분을 그대로 적용한다.
3. 교차검증을 한다면 각 fold 안에서 PCA를 다시 fit한다.

전체 데이터로 PCA를 먼저 fit한 뒤 train/test를 나누면 테스트 분포 정보가 학습 전처리에 들어가 평가가 낙관적으로 왜곡될 수 있다.
...
components = Vt[:k].T
Z = X_centered @ components
print(k, cumulative[k - 1])
```

새 데이터는 반드시 학습 평균을 빼고 같은 `components`를 곱한다.

```python
X_new = np.array([[3.0, 0.5]])
Z_new = (X_new - mean) @ components
```
...
샘플 수보다 특징 수가 훨씬 큰 경우에는 공분산 행렬 $d\times d$를 직접 만들면 메모리가 커진다. 중심화 데이터에 대한 truncated SVD를 직접 쓰거나, 스트리밍 데이터에는 incremental PCA를 사용한다.
...
- 표준화가 항상 좋은 것은 아니다. 단위와 크기 자체가 의미 있는 물리량이면 중심화만 하는 편이 맞을 수 있다.
- PCA 결과는 선형 구조만 본다. 곡선형 다양체나 군집 구조는 2차원 PCA 그림에서 잘못 보일 수 있다.
...
- train/test split 이전에 PCA를 fit했을 때와 파이프라인 안에서 fit했을 때 검증 점수가 달라지는지 실험하라.
- 원래 특징 스케일을 바꿨을 때 첫 주성분 방향이 어떻게 바뀌는지 설명하라.
