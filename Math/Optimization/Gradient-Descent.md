...
1차원 이차함수 $f(x)=(x-3)^2$에서는 갱신식이

$$x_{t+1}=x_t-\eta 2(x_t-3)=(1-2\eta)x_t+6\eta$$

가 된다. 수렴하려면 오차 $e_t=x_t-3$에 대해 $e_{t+1}=(1-2\eta)e_t$이고, $|1-2\eta|<1$이어야 하므로 $0<\eta<1$이 필요하다. $\eta=1.1$이 발산하는 이유가 여기서 보인다.
...
수렴 trace를 직접 보면 감이 잡힌다.

```python
def trace(lr, x=0.0, steps=5):
    out = []
    for _ in range(steps):
...
print(trace(0.1))  # [0.0, 0.6, 1.08, 1.464, 1.7712]
print(trace(0.9))  # [0.0, 5.4, 1.08, 4.536, 1.7712]
```

둘 다 수렴하지만 `0.9`는 최솟값 양쪽을 오가며 접근한다. 큰 학습률은 빠를 수 있지만 진동과 발산 위험을 키운다.
...
워크드 예제: 표본 10,000개, 특성 100개, 배치 GD 1,000스텝이면 기울기 계산만 대략 `1,000 * 10,000 * 100 = 10^9` 단위 연산 규모다. 미니배치 크기 100이면 한 스텝은 작아지지만 더 많은 noisy step이 필요할 수 있다.
