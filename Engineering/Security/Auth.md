...
인증 시스템은 로그인 성공보다 수명주기 관리가 더 어렵다. 세션 생성, 권한 상승, 토큰 갱신, 로그아웃, 기기 분실, 의심 활동, 키 회전, 계정 폐쇄가 모두 정책에 포함되어야 한다. 특히 refresh token은 장기 자격 증명이므로 저장 위치, 회전, 재사용 탐지, 폐기 경로를 명확히 둔다.

Access token은 짧게, refresh token은 더 강하게 보호하고, 고위험 동작에는 재인증이나 step-up authentication을 요구한다.

### 인가 모델링

RBAC는 역할 중심이라 운영이 쉽지만 객체 관계를 세밀하게 표현하기 어렵다. ABAC는 속성 기반 정책을 표현하기 좋지만 정책 추론과 테스트가 복잡하다. ReBAC은 사용자-자원 관계를 잘 표현하지만 그래프 일관성과 캐시 무효화가 중요해진다.

좋은 인가 설계는 "누가", "어떤 자원에", "어떤 동작을", "어떤 맥락에서" 할 수 있는지 명시한다. 정책 변경은 코드 변경처럼 리뷰와 회귀 테스트가 필요하다.

### 공격 표면

인증/인가는 피싱, 세션 고정, 토큰 탈취, redirect URI 조작, 권한 상승, IDOR/BOLA, confused deputy 공격을 함께 고려해야 한다. 서버는 클라이언트가 보낸 사용자 ID나 역할을 신뢰하지 않고, 신뢰된 세션·토큰에서 subject를 꺼내 자원 단위 정책을 평가한다.
