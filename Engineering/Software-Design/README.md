# 소프트웨어 설계 (Software Design)

> 변경에 강하고 읽기 쉬운 코드를 설계하는 원칙.

**선수지식**: [Programming/](../../Programming/), OOP 개념

---

## 읽는 법

- 링크가 걸린 `Draft` 이상 문서는 지금 읽을 수 있는 본문이다.
- `Planned` 파일명은 앞으로 채울 예정 주제이며 아직 본문 파일은 없다.

---

## 주제 목록

### 설계 원칙

| 주제 | 파일 | Status |
|---|---|---|
| SOLID 원칙 | SOLID.md | Planned |
| DRY / KISS / YAGNI | Design-Principles.md | Planned |
| 클린 코드 | Clean-Code.md | Planned |
| 리팩토링 | Refactoring.md | Planned |

### 디자인 패턴

| 주제 | 파일 | Status |
|---|---|---|
| 생성 패턴 (Singleton, Factory, Builder) | Creational-Patterns.md | Planned |
| 구조 패턴 (Adapter, Composite, Decorator) | Structural-Patterns.md | Planned |
| 행동 패턴 (Strategy, Observer, Command) | Behavioral-Patterns.md | Planned |

---

## 학습 순서

```text
Design-Principles → SOLID → Clean-Code → Refactoring
        ↓
Creational-Patterns → Structural-Patterns → Behavioral-Patterns
```

---

## 연관 섹션

- [Engineering/System-Design/](../System-Design/) — 시스템 수준 설계
- [Engineering/Testing/](../Testing/) — 설계와 테스트 가능성
