...
현재 바로 읽을 수 있는 본문은 프로그래밍 기초, 논리, 기본 자료구조, 기본 알고리즘까지의 최소 경로다. 아래 전체 로드맵은 최종 목표이며, 일부 완료 기준은 아직 `Planned` 문서가 채워져야 닫힌다.

## 순서

현재 바로 읽을 수 있는 경로는 다음 순서다.
...
현재 경로를 다 읽은 뒤 입문자 완료 기준을 채우려면 다음 예정 주제가 추가로 필요하다.
